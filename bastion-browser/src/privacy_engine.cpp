...
WebKitWebContext* PrivacyEngine::create_context() {
    ...
    std::cout << "[Bastion] Ephemeral context created.\n";
    std::cout << "[Bastion] Proxy mode: " << proxy_->description() << "\n";
    return ctx;
}
...
void PrivacyEngine::harden_context(WebKitWebContext* ctx) const {
    ...
    // Note: the "bastion://" URI scheme is registered by BrowserWindow
    // (it needs access to AppState).
}
...
WebKitSettings* PrivacyEngine::create_settings() {
    ...
    std::cout << "[Bastion] Settings hardened.\n";
    return s;
}
...
WebKitUserContentManager* PrivacyEngine::create_content_manager(WebKitWebContext* ctx) {
    ...
    std::cout << "[Bastion] Content manager configured.\n";
    return ucm;
}
