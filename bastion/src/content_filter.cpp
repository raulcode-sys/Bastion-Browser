...
void ContentFilter::on_content_rules_compiled(GObject* src, GAsyncResult* res, gpointer data) {
    ...
    if (error) {
        g_printerr("[Phantom] Content filter compile error: %s\n", error->message);
        g_error_free(error);
        delete rctx;
        return;
    }
    ...
    std::cout << "[Phantom] Content filter rules compiled and applied.\n";
    delete rctx;
}
...
void ContentFilter::compile_and_apply(WebKitUserContentManager* ucm, WebKitWebContext* ctx) {
    ...
    const gchar* store_path = "/tmp/phantom-cf-store";
    ...
    webkit_user_content_filter_store_save(
        store, "phantom-blocklist", rule_bytes,
        nullptr,
